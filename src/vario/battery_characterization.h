...
namespace battery_characterization {
  ...
  constexpr uint16_t CHARGE_PATH_MILLIOHMS = 190;
  ...
  constexpr uint16_t estimatedChargeCurrentMA(uint16_t batteryMV, bool hasCurrentSense,
    ...
    // Older boards cannot measure charge current. Approximate the constant-current phase as 500mA,
    // then taper linearly over the 4.0-4.2V constant-voltage phase.
    if (batteryMV <= 4000) return 500;
    if (batteryMV >= 4200) return 0;
    return static_cast<uint16_t>((4200 - batteryMV) * 500 / 200);
  }
