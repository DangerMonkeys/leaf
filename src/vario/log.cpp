...
void log_update() {
  ...
  if (!flight && !weAreFlying) {
    ...
    if (flightTimer_autoStart()) {
      ...
      if (leafSettings.log_autoStart) flightTimer_start();  // start a log if auto-start is on
    } else {
    ...
    if (flightTimer_autoStop()) {
      if (leafSettings.log_autoStop) flightTimer_stop();  // stop the log if auto-stop is on
    }
  }
...
void flightTimer_start() {
  ...
  switch (leafSettings.log_format) {
    case LOG_FORMAT_KML:
      flight = &kmlFlight;
      break;
    case LOG_FORMAT_IGC:
  ...
  if (leafSettings.vario_altSyncToGPS) baro.syncToGPSAlt();
  ...
  FanetRadio::getInstance().begin(leafSettings.fanet_region);
}
