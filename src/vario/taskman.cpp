...
void main_CHARGE_loop() {
  if (chargeman_doTasks) {
    ...
    */
    ...
    if (goToSleep && leafSettings.system_ecoMode) {  // don't allow sleep if ECO_MODE is off
      ...
      if (leafSettings
              .system_ecoMode) {  // TODO: this is doubling the condition since we already check
                                  // ECO_MODE in
