...
void display_distance(uint8_t cursor_x, uint8_t cursor_y, double distance) {
  ...
  if (unitsSmall && leafSettings.units_distance) u8g2.print((char)128);
  if (!unitsSmall && leafSettings.units_distance) u8g2.print((char)130);
  if (unitsSmall && !leafSettings.units_distance) u8g2.print((char)127);
  if (!unitsSmall && !leafSettings.units_distance) u8g2.print((char)129);
}
...
void display_heading(uint8_t cursor_x, uint8_t cursor_y, bool degSymbol) {
  ...
  if (!gpsFixInfo.fix) {  // blank out heading if no gps fix
    u8g2.print("---");
  } else if (leafSettings.units_heading) {  // Cardinal heading direction
...
void display_GPS_icon(uint8_t x, uint8_t y) {
  ...
  if (leafSettings.gpsMode == 0) {    // GPS Off
    u8g2.print((char)44);             // GPS icon with X through it
  } else if (leafSettings.gpsMode) {  // GPS not-off
    if (gpsFixInfo.fix) {
...
void display_headerAndFooter(bool timerSelected, bool showTurnArrows) {
  ...
  u8g2.setFont(leaf_icons);
  if (leafSettings.vario_quietMode && !flightTimer_isRunning() && leafSettings.vario_volume != 0) {
    u8g2.print((char)('I' + 4));
  } else {
