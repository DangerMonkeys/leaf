...
void navigatePage_draw() {
  ...
  do {
    ...
    u8g2.setDrawColor(0);
    u8g2.setFontMode(1);
    if (leafSettings.units_climb) {  // FPM units
    ...
    // alt
    display_alt_type(49, 109, leaf_8x14, leafSettings.disp_navPageAltType);
    // alt label
    u8g2.setDrawColor(0);
    u8g2.setCursor(78, topOfFrame + 2 * nav_r + 5);
    print_alt_label(leafSettings.disp_navPageAltType);
  ...
  switch (navigatePage_cursorPosition) {
    ...
    case cursor_navigatePage_alt1:
      switch (button) {
        ...
        case Button::CENTER:
          if (state == RELEASED)
            leafSettings.adjustDisplayField_navPage_alt(Button::CENTER);
          else if (state == HELD && count == 1 && leafSettings.disp_navPageAltType == altType_MSL) {
