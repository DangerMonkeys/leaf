...
void display_init(void) {
  ...
  display_setContrast(leafSettings.disp_contrast);
...
void display_turnPage(uint8_t action) {
  ...
  switch (action) {
    ...
    case page_next:
      ...
      if (display_page == page_thermalSimple && !leafSettings.disp_showThmPage) display_page++;
      if (display_page == page_thermal && !leafSettings.disp_showThmAdvPage) display_page++;
      if (display_page == page_nav && !leafSettings.disp_showNavPage) display_page++;
    ...
    case page_prev:
      ...
      if (display_page == page_thermal && !leafSettings.disp_showThmAdvPage) display_page--;
      if (display_page == page_thermalSimple && !leafSettings.disp_showThmPage) display_page--;
      if (display_page == page_debug && !leafSettings.disp_showDebugPage)
...
void display_update() {
  ...
  if (leafSettings.system_showWarning && showWarning) {
    warningPage_draw();
    return;
  } else {
