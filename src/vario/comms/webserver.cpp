...
$('pilotNew').onclick=()=>{profiles.active_pilot_id=null;$('pilotList').value='';$('pilotName').value='';$('pilotEmail').value='';$('leafLogCode').value='';$('leafLogPanel').classList.remove('active');setSnaps();$('pilotName').focus();msg('pilotMsg','Enter pilot name, then Save Profile.')};
$('gliderNew').onclick=()=>{profiles.active_glider_id=null;$('gliderList').value='';['gliderBrand','gliderModel','gliderSize','gliderDisplay'].forEach(x=>$(x).value='');setSnaps();$('gliderModel').focus();msg('gliderMsg','Enter glider model, then Save Profile.')};
...
$('gliderSave').onclick=()=>{let model=clean($('gliderModel').value),size=clean($('gliderSize').value),display=clean($('gliderDisplay').value);if(!model){msg('gliderMsg','Enter glider model before saving.');return}let g=selectedGlider();if(!g){g={id:newId(),model:''};profiles.gliders.push(g);profiles.active_glider_id=g.id}g.brand=clean($('gliderBrand').value);g.model=model;g.size=size;g.display_name=display||[model,size].filter(Boolean).join(' ');save().then(()=>msg('gliderMsg','Glider profile saved.')).catch(()=>msg('gliderMsg','Unable to save glider.'))};
$('pilotDelete').onclick=()=>{let p=selectedPilot();if(!p)return;profiles.pilots=profiles.pilots.filter(x=>x.id!=p.id);profiles.active_pilot_id=profiles.pilots.length?profiles.pilots[0].id:null;save().then(()=>msg('pilotMsg','Pilot profile deleted.')).catch(()=>msg('pilotMsg','Unable to delete pilot.'))};
$('gliderDelete').onclick=()=>{let g=selectedGlider();if(!g)return;profiles.gliders=profiles.gliders.filter(x=>x.id!=g.id);profiles.active_glider_id=profiles.gliders.length?profiles.gliders[0].id:null;save().then(()=>msg('gliderMsg','Glider profile deleted.')).catch(()=>msg('gliderMsg','Unable to delete glider.'))};
