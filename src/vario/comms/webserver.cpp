...
let scaleMode='';
function renderScales(rate){let mode=(rate>5?'high':'low')+'|'+(rate<-5?'high':'low');if(mode==scaleMode)return;scaleMode=mode;let climb=rate>5?[10,9,8,7,6,5]:[5,4,3,2,1,0],sink=rate<-5?[-5,-6,-7,-8,-9,-10]:[0,-1,-2,-3,-4,-5];q('#vsClimbScale').innerHTML=climb.map((v,i)=>`<span style="top:${i*10}%">${v>0?'+':''}${v}</span>`).join('');q('#vsSinkScale').innerHTML=sink.map((v,i)=>`<span style="top:${50+i*10}%">${v}</span>`).join('')}
function setRate(rate){q('#vsRate').textContent=(rate>0?'+':'')+rate.toFixed(1);renderScales(rate);let c=q('#vsClimbFill'),s=q('#vsSinkFill');c.style.height=s.style.height='0';if(rate>0){let m=Math.min(rate,10),h=m<=5?m/5*50:Math.max(0,(10-m)/5*50);c.style.top=(m<=5?50-h:0)+'%';c.style.height=h+'%'}else if(rate<0){let m=Math.min(-rate,10),h=m<=5?m/5*50:Math.max(0,(10-m)/5*50);s.style.top=(m<=5?50:100-h)+'%';s.style.height=h+'%'}}
function animateRate(target,ramp,hold){let start=performance.now(),duration=ramp*2+hold;function frame(now){let e=now-start,rate=e<ramp?target*e/ramp:e<ramp+hold?target:e<duration?target*(duration-e)/ramp:0;setRate(rate);if(e<duration)requestAnimationFrame(frame);else{running=false;testButtons();message('Test complete.')}}requestAnimationFrame(frame)}
