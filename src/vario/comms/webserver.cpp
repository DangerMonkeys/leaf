...
namespace {
  ...
  const char* emptyProfilesJson() {
    return "{\"schema\":\"leaf.profiles\",\"schema_version\":\"v0.1.0\","
           ...
           "\"leaf_log\":{\"pilot_id\":null,\"account_email\":null,\"token\":null},"
           "\"pilots\":[],\"gliders\":[]}";
  }
  ...
  bool profilesRequestLooksValid(const String& body) {
    ...
    JsonVariant leafLog = doc["leaf_log"];
    if (!leafLog.isNull()) {
      if (!leafLog.is<JsonObject>()) return false;
      JsonObject leafLogObj = leafLog.as<JsonObject>();
      String pilotId = leafLogObj["pilot_id"] | "";
      String accountEmail = leafLogObj["account_email"] | "";
      String token = leafLogObj["token"] | "";
      pilotId.trim();
      accountEmail.trim();
      token.trim();
      if (pilotId.length() > 20 || accountEmail.length() > 80 || token.length() > 180) return false;
      if (!pilotId.isEmpty()) {
        bool leafLogPilotFound = false;
        for (JsonObject item : pilots) {
          String id = item["id"] | "";
          id.trim();
          if (id == pilotId) {
            leafLogPilotFound = true;
            break;
          }
        }
        if (!leafLogPilotFound) return false;
      }
    }
  ...
  bool storeLeafLogToken(const String& token) {
    if (token.length() < 5 || token.substring(0, 4) != "llk_") return false;

    JsonDocument doc;
    if (!loadProfilesForUpdate(doc)) return false;
    ...
    if (!profilesRequestLooksValid(body)) return false;
    return writeProfilesBody(body);
  }
  ...
  void pollLeafLogPair(WebServer& target) {
    ...
    token.trim();
    bool saved = false;
    if (status == "claimed" && !token.isEmpty()) {
      saved = storeLeafLogToken(token);
      leaf_log_pair_code = "";
...
let profiles={schema:'leaf.profiles',schema_version:'v0.1.0',active_pilot_id:null,active_glider_id:null,leaf_log:{pilot_id:null,account_email:null,token:null},pilots:[],gliders:[]},pilotSnap={},gliderSnap={},navPoints=[],loadedNavFile='',userWaypoints=[],userWaypointSnap='',selectedUserWaypointId='',editRoute=[],logState={prev:'',next:'',path:''},unitPrefs={alt_feet:false,climb_fpm:false,speed_mph:false,distance_miles:false,heading_cardinal:false,temp_f:false,time_12h:false},userStatus={mode:'',mac_address:''},previewState={points:[],lo:0,hi:0},leafLogActivationUrl='',leafLogPollTimer=0,leafLogBusy=false;
...
function leafLog(){return profiles.leaf_log||(profiles.leaf_log={pilot_id:null,account_email:null,token:null})}function leafLogSelectedPilot(){let l=leafLog();return profiles.pilots.find(p=>p.id==l.pilot_id)}function leafLogSync(){let l=leafLog();l.pilot_id=$('leafLogPilot').value||null;l.account_email=clean($('leafLogEmail').value)||null}function leafLogLinked(){return !!clean(leafLog().token)}function leafLogButtons(){let card=$('leafLogCard');if(!LEAF_LOG_ENABLED){card.style.display='none';return}card.style.display='block';leafLogSync();let l=leafLog(),hasPilot=!!leafLogSelectedPilot(),email=validEmail($('leafLogEmail').value),network=userStatus.mode=='network',linked=leafLogLinked();if(leafLogBusy){$('leafLogStart').textContent='Getting Code...';$('leafLogStart').disabled=true;$('leafLogWifi').style.display='none';msg('leafLogStatus','Getting activation code...');msg('leafLogMsg','Contacting Leaf Log. This can take a few seconds.');return}$('leafLogStart').textContent=linked?'Linked':'Get Activation Code';$('leafLogWifi').style.display=(network||linked)?'none':'block';$('leafLogStart').disabled=linked||!hasPilot||!email||!network;if(linked){$('leafLogPanel').classList.remove('active');msg('leafLogMsg','');msg('leafLogStatus','Linked'+(l.account_email?' as '+l.account_email:''));}else if(!profiles.pilots.length)msg('leafLogStatus','Create a pilot profile first.');else if(!hasPilot)msg('leafLogStatus','Select the default pilot.');else if(!email)msg('leafLogStatus','Enter the Leaf Log account email.');else if(!network)msg('leafLogStatus','Join a WiFi network to link.');else msg('leafLogStatus','Ready to link')}async function startLeafLog(){leafLogSync();leafLogButtons();if($('leafLogStart').disabled)return;leafLogBusy=true;leafLogButtons();try{await save();let r=await fetch('/api/leaf-log/pair/start',{method:'POST'}),d=await r.json().catch(()=>({}));if(!r.ok||!d.ok)throw d;leafLogBusy=false;leafLogActivationUrl=d.activation_url||'';$('leafLogCodeText').textContent=d.code?'Code '+d.code:'Open Leaf Log to continue';$('leafLogPanel').classList.add('active');$('leafLogStart').textContent='Get Activation Code';msg('leafLogStatus','Activation code ready.');msg('leafLogMsg','Open Leaf Log, sign in, then approve this device.');pollLeafLog()}catch(x){leafLogBusy=false;msg('leafLogMsg',x&&x.detail?x.detail:'Unable to start Leaf Log linking.');leafLogButtons()}}function pollLeafLog(){if(leafLogPollTimer)clearTimeout(leafLogPollTimer);leafLogPollTimer=setTimeout(async()=>{try{let r=await fetch('/api/leaf-log/pair/poll',{method:'POST'}),d=await r.json().catch(()=>({}));if(!r.ok)throw d;if(d.status=='pending'){msg('leafLogMsg','Waiting for Leaf Log approval...');pollLeafLog();return}if(d.status=='claimed'&&d.saved){msg('leafLogMsg','Leaf Log linked.');await loadProfiles();return}msg('leafLogMsg','Linking ended: '+(d.status||'unknown')+'.')}catch(x){msg('leafLogMsg',x&&x.detail?x.detail:'Unable to check Leaf Log approval.')}} ,3000)}function leafLogUrl(){return leafLogActivationUrl}function versionText(s){let fw=s.firmware_display_version||'',hw=s.hardware_display_version||'';if(!fw){let a=(s.firmware_version||'').split('+');fw=a[0]||'unknown';hw=a[1]||hw||'';if(fw[0]!='v')fw='v'+fw;if(hw&&hw[0]=='h')hw=hw.slice(1);if(hw&&hw[0]!='v')hw='v'+hw}return `firmware: ${fw}`+(hw?`\nhardware: ${hw}`:'')}
