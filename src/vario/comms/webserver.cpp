...
.settings-status-tools{display:flex;align-items:center;justify-content:flex-end;gap:14px}.settings-gear{display:flex;align-items:center;justify-content:center;flex:0 0 42px;width:42px;height:42px;padding:0;background:transparent;color:#e2e7dc;border:1px solid #e2e7dc;box-shadow:none}.settings-gear svg{width:24px;height:24px;fill:none;stroke:currentColor;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round}.vs-step{grid-template-columns:30px minmax(58px,110px) auto 30px;justify-content:start}.vs-step button{width:30px}.vs-step.has-tone{grid-template-columns:30px minmax(58px,110px) auto 30px 34px}.vs-step .vs-tone-play{width:34px;background:var(--leaf);border-color:var(--leaf);color:#0b0d0b;font-size:17px}.vs-field input[type=range]{width:224px;max-width:100%;display:block}
</style>
<script>
(function(){
...
const sectionHeader=(kind,label)=>`<div class=vs-group-head><strong>${label}</strong><div class=vs-section-actions><button type=button class=hero data-test="${kind}">Test ${label}</button><button type=button class=hero data-save disabled>Save to Leaf</button><button type=button class=secondary data-defaults="${kind}">Defaults</button></div><p class="muted msg vs-section-msg" data-vs-msg></p></div>`;
...
function setForm(d){configureUnits(d.climb_fpm);qa('[data-vs]').forEach(e=>setControl(e,d[e.dataset.vs]));qa('[data-range]').forEach(r=>{let i=q(`[data-vs="${r.dataset.range}"]`);if(i)r.value=i.value});savedSignature=JSON.stringify(readForm());dirty=false;updateDerived();updateButtons();setRate('Climb',0);setRate('Sink',0)}
...
function validate(){qa('.vs-field').forEach(f=>f.classList.remove('invalid'));qa('[data-error]').forEach(e=>e.textContent='');let valid=true,fail=(key,text)=>{let input=q(`[data-vs="${key}"]`),field=input&&input.closest('.vs-field'),error=q(`[data-error="${key}"]`);if(field)field.classList.add('invalid');if(error)error.textContent=text;valid=false},climbStart=cmsAsShown(value('climb_start_cms')),climbContinuous=value('climb_continuous_cms'),climbMax=value('climb_max_cms'),climbStartTone=value('climb_start_hz'),climbMaxTone=value('climb_max_hz'),sinkStart=cmsAsShown(value('sink_alarm_cms')),sinkContinuous=value('sink_continuous_cms'),sinkMax=value('sink_max_cms'),sinkStartTone=value('sink_start_hz'),sinkMinTone=value('sink_min_hz');if(climbContinuous<=climbStart)fail('climb_continuous_cms',`Must be higher than Beeping starts at (${rate(climbStart)}).`);if(climbMax<=climbContinuous)fail('climb_max_cms',`Must be higher than Continuous above (${rate(climbContinuous)}).`);if(climbMaxTone<=climbStartTone)fail('climb_max_hz',`Must be higher than Starting tone (${climbStartTone} Hz).`);if(sinkContinuous>=sinkStart)fail('sink_continuous_cms',`Must be lower than Sink alarm starts at (${sinkStart==0?'Off':rate(sinkStart)}).`);if(sinkMax>=sinkContinuous)fail('sink_max_cms',`Must be lower than Continuous below (${rate(sinkContinuous)}).`);if(sinkMinTone>=sinkStartTone)fail('sink_min_hz',`Must be lower than Starting tone (${sinkStartTone} Hz).`);formValid=valid;return valid}
function updateDerived(){validate();let climbStart=cmsAsShown(value('climb_start_cms')),climbContinuous=value('climb_continuous_cms'),climbMax=value('climb_max_cms'),climbStartTone=value('climb_start_hz'),climbMaxTone=value('climb_max_hz'),climbTone=climbStartTone+(climbContinuous-climbStart)*(climbMaxTone-climbStartTone)/(climbMax-climbStart);q('#vsClimbDerived').textContent=Number.isFinite(climbTone)&&climbMax>climbStart?`Interpolated continuous tone: ${Math.round(climbTone)} Hz`:'';let sinkStart=cmsAsShown(value('sink_alarm_cms')),sinkContinuous=value('sink_continuous_cms'),sinkMax=value('sink_max_cms'),sinkStartTone=value('sink_start_hz'),sinkMinTone=value('sink_min_hz'),sinkTone=sinkStartTone-(sinkStart-sinkContinuous)*(sinkStartTone-sinkMinTone)/(sinkStart-sinkMax);q('#vsSinkDerived').textContent=Number.isFinite(sinkTone)&&sinkStart>sinkMax?`Interpolated continuous tone: ${Math.round(sinkTone)} Hz`:''}
function changed(){updateDerived();dirty=JSON.stringify(readForm())!==savedSignature;message(dirty?(formValid?'Unsaved changes — save to enable tests.':'Fix the highlighted settings before saving.'):'',dirty);updateButtons()}
function updateButtons(){qa('[data-save]').forEach(b=>b.disabled=running||!dirty||!formValid);qa('[data-test]').forEach(b=>b.disabled=running||dirty||!formValid);qa('[data-defaults]').forEach(b=>b.disabled=running);qa('[data-tone]').forEach(b=>b.disabled=running)}
...
async function test(kind){if(dirty){message('Save changes before testing.',true);return}running=true;updateButtons();message('Running '+kind+' test...');try{let r=await fetch('/api/settings/vario/test',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:kind})}),d=await r.json().catch(()=>({}));if(!r.ok)throw d;animateRate(cmsAsShown(Number(d.target_cms)),Number(d.ramp_ms)||15000,Number(d.hold_ms)||1500)}catch(e){running=false;updateButtons();message(e&&e.detail?e.detail:'Unable to start test.',true)}}
async function playTone(key,button){let frequency=Math.round(value(key));button.disabled=true;message(`Playing ${frequency} Hz...`);try{let r=await fetch('/api/settings/vario/tone',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({frequency_hz:frequency})}),d=await r.json().catch(()=>({}));if(!r.ok)throw d;setTimeout(()=>{button.disabled=false;message(dirty?'Unsaved changes — save to enable tests.':'',dirty)},500)}catch(e){button.disabled=false;message(e&&e.detail?e.detail:'Unable to play tone.',true)}}
function restore(kind){let prefix=kind+'_';qa('[data-vs]').forEach(e=>{let k=e.dataset.vs;if(k.startsWith(prefix))setControl(e,defaults[k])});qa('[data-range]').forEach(r=>{let i=q(`[data-vs="${r.dataset.range}"]`);if(i)r.value=i.value});changed();if(dirty)message(`${kind[0].toUpperCase()+kind.slice(1)} defaults loaded — save to apply.`,true)}
const snapped=(v,step)=>{let p=(String(step).split('.')[1]||'').length;return Number(v.toFixed(p))};
card.onclick=e=>{let step=e.target.closest('[data-step]');if(step){let i=q(`[data-vs="${step.dataset.for}"]`),amount=Number(step.dataset.step),v=snapped(Number(i.value)+amount,Math.abs(amount));i.value=Math.max(Number(i.min),Math.min(Number(i.max),v));let r=q(`[data-range="${step.dataset.for}"]`);if(r)r.value=i.value;changed();return}let tone=e.target.closest('[data-tone]');if(tone){playTone(tone.dataset.tone,tone);return}let t=e.target.closest('[data-test]');if(t){test(t.dataset.test);return}if(e.target.closest('[data-save]')){save();return}let d=e.target.closest('[data-defaults]');if(d)restore(d.dataset.defaults)};
...
card.onchange=e=>{let i=e.target;if(i.matches('input[type=number][data-vs]')){let step=Number(i.step)||1;i.value=snapped(Number(i.value),step);let r=q(`[data-range="${i.dataset.vs}"]`);if(r)r.value=i.value;changed()}};
